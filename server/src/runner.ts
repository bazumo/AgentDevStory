...
export class AgentRunner {
  ...
  async run(issue: LinearIssue, project: LinearProject, session: AgentSession, callbacks: RunnerCallbacks, memoryContext = ""): Promise<void> {
    ...
    const prompt = buildPrompt(issue, project, memoryContext);
    await this.runCodexPrompt(workspacePath, prompt, callbacks);
  }

  async runPromptInSession(session: AgentSession, message: string, callbacks: RunnerCallbacks, memoryContext = ""): Promise<void> {
    if (!this.canRunRealAgents()) {
      throw new Error("AGENTDEVSTORY_TARGET_REPO and CODEX_CMD are required for real agent runs");
    }
    if (!session.workspacePath) {
      throw new Error("Session does not have a workspace yet");
    }

    const prompt = buildFollowUpPrompt(session, message, memoryContext);
    await this.runCodexPrompt(session.workspacePath, prompt, callbacks);
  }

  private async runCodexPrompt(workspacePath: string, prompt: string, callbacks: RunnerCallbacks): Promise<void> {
    ...
    proc.stdin.end();
    callbacks.onEvent(event("lifecycle", "Prompt sent to Codex"));
...
function buildFollowUpPrompt(session: AgentSession, message: string, memoryContext = ""): string {
  return [
    `Continue work for AgentDevStory session ${session.id}.`,
    `Linear issue: ${session.issueIdentifier}`,
    session.workspacePath ? `Workspace: ${session.workspacePath}` : null,
    "",
    "User follow-up:",
    message,
    memoryContext ? "\nRelevant g-brain memory:\n" + memoryContext : null,
    "",
    "Use the current workspace state. Make the requested changes, run appropriate checks, and summarize the result."
  ]
    .filter((line): line is string => line !== null)
    .join("\n");
}

function buildPrompt(issue: LinearIssue, project: LinearProject, memoryContext = ""): string {
  return [
